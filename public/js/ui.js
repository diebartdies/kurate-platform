...
function ensureResponsiveCss() {
    if (document.getElementById('KuraTe-responsive-css')) return;
    if (document.querySelector('link[rel="stylesheet"][href*="responsive.css"]')) return;

    const link = document.createElement('link');
    link.id = 'KuraTe-responsive-css';
    link.rel = 'stylesheet';
...
export function injectGlobalStyles() {
    ...
    if (!document.getElementById('KuraTe-global-styles')) {
        const style = document.createElement('style');
        style.id = 'KuraTe-global-styles';
        style.textContent = `
            .fileteado-section {
...
export function injectPlausible() {
    ...
    script.defer = true;
    script.setAttribute('data-domain', 'KuraTe.drsrv.net.ar');
...
export function initGlobalTopBar() {
    ...
    brandLink.setAttribute('aria-label', 'KuraTe');
    brandLink.innerHTML = `
        ...
        <span class="brand-text" style="font-family: 'Playfair Display', serif; font-weight: 900; letter-spacing: 1px; color: white;">KuraTe</span>
    `;
