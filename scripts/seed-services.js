...
async function seed() {
  ...
  const raw = fs.readFileSync(path.join(__dirname, '..', 'public', 'docs', 'Arbol-entoro-categoria-dispositio-macas.txt'), 'utf8');
  ...
  for (const block of blocks) {
    ...
    try {
      ...
      console.error('Failed to parse block:', block.substring(0, 80));
      continue;
    }
    ...
    for (const cat of data.categorias) {
      for (const dev of cat.dispositivos) {
        const docPath = buildPath(area, cat.nombre, dev.nombre);
        allDocs.push({
          area,
          category: cat.nombre,
          device: dev.nombre,
  ...
  console.log(`Parsed ${allDocs.length} devices from file`);
